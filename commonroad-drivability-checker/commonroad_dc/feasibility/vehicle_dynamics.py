...
class VehicleParameterMapping(Enum):
    ...
    """
    MOTORVESSEL = parameters_vehicle1()
    SAILINGVESSEL = parameters_vehicle2()
    CARGO_SHIP = parameters_vehicle3()
...
class PointMassDynamics(VehicleDynamics):
    ...
    def _array_to_state(self, x: np.array, time_step: int) -> State:
        ...
        values = {
            ...
            'velocity': x[2],
            'velocity_y': x[3],
            'orientation': math.atan2(x[3], x[2]) #TODO: check if present otherwise revert
        }
        return State(**values, time_step=time_step)
