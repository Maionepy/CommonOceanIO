...
def lane_sections(fairway_network):
    ...
    while fairway_id_list != []:
        ...
        current = fairway_network.find_waters_by_id(id)
        direction = True
        ...
        while (direction and current.adj_right != None) or (not direction and current.adj_left != None):
            ...
            current = waters_network.find_lanelet_by_id(next_id)
        """
        ...
        while (direction and current.adj_left != None) or (not direction and current.adj_right != None):
            ...
            current = waters_network.find_lanelet_by_id(next_id)
            if next_id in fairway_id_list:
                fairway_id_list.remove(next_id)
            add_lateral_vertices(current, direction)
        """
