...
from commonocean.scenario.traffic_sign import TrafficLightState, TrafficLight, TrafficLightDirection
...
def collect_center_line_colors(fairway_network: WatersNetwork, traffic_lights: List[TrafficLight], time_step) \
        -> Dict[int, TrafficLightState]:
    """Collects traffic light states that each water is affected by."""
    ...
    l2int = fairway_network.map_inc_lanelets_to_intersections
    l2state = {}
    for lanelet in fairway_network.lanelets:
        intersection = l2int[lanelet.lanelet_id] if lanelet.lanelet_id in l2int else None
        for tl_id in lanelet.traffic_lights:
            tl = fairway_network.find_traffic_light_by_id(tl_id)
