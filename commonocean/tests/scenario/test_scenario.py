...
from commonocean.scenario.waters import Waters, WatersNetwork, LineMarking
from commonocean.scenario.obstacle import *
from commonocean.scenario.scenario import Scenario, Environment, TimeOfDay, Time, SeaState, Weather, Location, \
    ScenarioID, GeoTransformation
# from commonocean.scenario.traffic_sign import TrafficSign, TrafficSignElement, TrafficLight, TrafficLightCycleElement, TrafficLightState
...
class TestScenario(unittest.TestCase):
    def setUp(self):
        ...
        self.waters1 = Waters(np.array([[0.0, 0.0], [1.0, 0.0], [2, 0]]), np.array([[0.0, 1], [1.0, 1], [2, 1]]),
                                np.array([[0.0, 2], [1.0, 2], [2, 2]]), 100, [101], [101], None, {300, 301, 302}, {200, 201, 202})
        self.waters1.add_static_obstacle_to_water(0)
        self.waters2 = Waters(np.array([[0.0, 0.0], [1.0, 0.0], [2, 0]]), np.array([[0.0, 1], [1.0, 1], [2, 1]]),
                                np.array([[0.0, 2], [1.0, 2], [2, 2]]), 101, [100], [100], None, {301, 302, 303}, {201, 202, 203})
        self.waters1.add_dynamic_obstacle_to_water(2, 0)
        self.waters1.add_dynamic_obstacle_to_water(2, 1)
        self.waters_network = WatersNetwork().create_from_waters_list(list([self.waters1, self.waters2]))
        ...
        self.static_obs_on_waters = StaticObstacle(0, ObstacleType.UNKNOWN, obstacle_shape=self.circ,
        ...
        self.environment = Environment(Time(2022, 1, 1, 12, 15), TimeOfDay.NIGHT, Weather.SNOW, SeaState.CALM)
        self.location = Location(geo_name_id=123, gps_latitude=456, gps_longitude=789, environment=self.environment)

        scenario_id_str = "C-USA_US101-33_2_T-1"
        scenario_id = ScenarioID.from_benchmark_id(scenario_id_str, SCENARIO_VERSION)

        self.scenario = Scenario(0.1, location=self.location, scenario_id=scenario_id)
    ...
    def test_add_objects(self):
        ...
        expected_id_waters1 = self.waters1.waters_id
        expected_id_waters2 = self.waters2.waters_id
        ...
        self.scenario.add_objects(self.waters_network)
        ...
        self.assertEqual(expected_id_waters1, self.scenario.waters_network.waters[0].waters_id)
        self.assertEqual(expected_id_waters2, self.scenario.waters_network.waters[1].waters_id)
        # self.assertEqual(expected_id_traffic_sign, self.scenario.waters_network.traffic_signs[0].traffic_sign_id)
        # self.assertEqual(expected_id_traffic_light, self.scenario.waters_network.traffic_lights[0].traffic_light_id)
        # self.assertEqual(expected_id_intersection, self.scenario.waters_network.intersections[0].intersection_id)
        # self.assertEqual(expected_id_intersection_incoming1,
        #                  self.scenario.waters_network.intersections[0].incomings[0].incoming_id)
        # self.assertEqual(expected_id_intersection_incoming2,
        #                  self.scenario.waters_network.intersections[0].incomings[1].incoming_id)
    ...
    def test_remove_obstacle(self):
        ...
        expected_id_waters1 = self.waters_network.waters[0].waters_id
        expected_id_waters2 = self.waters_network.waters[1].waters_id
        ...
        with self.assertRaises(AssertionError):
            self.scenario.remove_obstacle(self.waters1)
            self.scenario.remove_obstacle(self.static_obs)

    # Not applicable to CommonOcean (does not have remove_hanging_waters_members, traffic sign/light and intersection)

    # def test_remove_hanging_waters_members(self):
    #     self.scenario.add_objects([self.waters1, self.waters2])
    ...
    #     self.scenario.remove_hanging_waters_members([self.waters1])
    #     self.assertEqual(self.scenario.waters_network._traffic_lights.keys(), {201, 202, 203})
    #     self.assertEqual(self.scenario.waters_network._traffic_signs.keys(), {301, 302, 303})
    ...
    #     self.assertEqual(len(self.scenario.waters_network.waters), 2)
    #     self.assertEqual(len(self.scenario.waters_network.waters), 2)
    #     self.scenario.remove_waters(self.waters1)
    #     self.assertEqual(len(self.scenario.waters_network.waters), 1)
    #     self.scenario.remove_waters(self.waters2)
    #     self.assertEqual(len(self.scenario.waters_network.waters), 0)
    #     self.assertFalse(self.scenario._is_object_id_used(self.waters1.waters_id))
    #     self.assertFalse(self.scenario._is_object_id_used(self.waters2.waters_id))
    #     self.scenario.add_objects(self.waters2)  # add again to check whether ID was removed successfully
    #     self.assertEqual(len(self.scenario.waters_network.waters), 1)

    #     with self.assertRaises(AssertionError):
    #         self.scenario.remove_waters(self.traffic_light)
    #         self.scenario.remove_waters(self.static_obs)

    # def test_remove_traffic_sign(self):
    #     self.scenario.add_objects(self.traffic_sign, set())

    #     self.assertEqual(len(self.scenario.waters_network.traffic_signs), 1)
    #     self.scenario.remove_traffic_sign(self.traffic_sign)
    #     self.assertEqual(len(self.scenario.waters_network.traffic_signs), 0)
    ...
    #     self.assertEqual(len(self.scenario.waters_network.traffic_lights), 1)
    #     self.scenario.remove_traffic_light(self.traffic_light)
    #     self.assertEqual(len(self.scenario.waters_network.traffic_lights), 0)
    ...
    #     self.assertEqual(len(self.scenario.waters_network.intersections), 1)
    #     self.scenario.remove_intersection(self.intersection)
    #     self.assertEqual(len(self.scenario.waters_network.intersections), 0)
    ...
    def test_obstacle_by_id(self):
        ...
        static_obs2 = StaticObstacle(0, ObstacleType.UNKNOWN, obstacle_shape=self.circ,
                                     initial_state=self.init_state)
        static_obs3 = StaticObstacle(5000, ObstacleType.MOTORVESSEL, obstacle_shape=self.circ, initial_state=self.init_state)
    ...
    def test_obstacles_by_role_and_type(self):
        ...
        static_obs2 = StaticObstacle(2, ObstacleType.UNKNOWN, obstacle_shape=self.circ,
                                     initial_state=self.init_state)
        static_obs3 = StaticObstacle(3, ObstacleType.MOTORVESSEL, obstacle_shape=self.circ, initial_state=self.init_state)
        dyn_set_obs1 = DynamicObstacle(4, ObstacleType.UNKNOWN,
                                       initial_state=self.traj_pred.trajectory.state_at_time_step(0),
                                       prediction=self.set_pred, obstacle_shape=self.rectangle)
        dyn_set_obs2 = DynamicObstacle(5, ObstacleType.MOTORVESSEL,
                                       initial_state=self.traj_pred.trajectory.state_at_time_step(0),
                                       prediction=self.set_pred, obstacle_shape=self.rectangle)
        ...
        output_three = self.scenario.obstacles_by_role_and_type(None, ObstacleType.MOTORVESSEL)
        output_four = self.scenario.obstacles_by_role_and_type(ObstacleRole.DYNAMIC, ObstacleType.MOTORVESSEL)
        ...
        output_five = self.scenario.obstacles_by_role_and_type(None, ObstacleType.MOTORVESSEL)
    ...
    def test_obstacles_by_position_intervals(self):
        ...
        static_obs3 = StaticObstacle(3, ObstacleType.MOTORVESSEL, obstacle_shape=self.circ, initial_state=init_state3)
        static_obs4 = StaticObstacle(4, ObstacleType.MOTORVESSEL, obstacle_shape=self.circ, initial_state=init_state4)
    ...
    def test_is_object_id_used(self):
        ...
        static_obs2 = StaticObstacle(-10, ObstacleType.UNKNOWN, obstacle_shape=self.circ,
                                     initial_state=self.init_state)
        waters1 = Waters(np.array([[0.0, 0.0], [1.0, 0.0], [2, 0]]), np.array([[0.0, 1], [1.0, 1], [2, 1]]),
                           np.array([[0.0, 2], [1.0, 2], [2, 2]]), 100, [101], [101])

        waters2 = Waters(np.array([[0.0, 0.0], [1.0, 0.0], [2, 0]]), np.array([[0.0, 1], [1.0, 1], [2, 1]]),
                           np.array([[0.0, 2], [1.0, 2], [2, 2]]), 101, [100], [100])

        self.scenario.add_objects(waters1)

        self.assertTrue(self.scenario._is_object_id_used(100))

        self.scenario.add_objects(waters2)
    ...
    def test_location(self):
        ...
        exp_env_seastate = SeaState.CALM
        ...
        self.assertEqual(exp_env_seastate, self.scenario.location.environment.seastate)


    # It is not being tested because assign_obstacles_to_lanelets is probably with the wrong name and implementation
    ...
    #                                    obstacle_shape=self.rectangle, initial_shape_waters_ids=None)
    #     sc = Scenario(dt=0.1)
    ...
    #     waters = Waters(right_vertices=right_vertices, left_vertices=left_vertices, center_vertices=center_vertices,
    #                       waters_id=1)
    #     sc.add_objects([waters, dyn_traj_obs])
    #     scenario_tmp: Scenario = deepcopy(sc)
    #     # assign all time steps
    #     scenario_tmp.assign_obstacles_to_waters(time_steps=None)
    #     exp_dynamic_obstacles_on_waters_zero = {0: {2}, 1: {2}}
    #     exp_dynamic_waters_of_obstacle = {0: {1}, 1: {1}}
    #     self.assertEqual(exp_dynamic_obstacles_on_waters_zero,
    #                      scenario_tmp.waters_network.find_waters_by_id(1).dynamic_obstacles_on_waters)
    #     self.assertEqual(exp_dynamic_waters_of_obstacle[0], scenario_tmp.obstacle_by_id(2).initial_center_waters_ids)
    #     self.assertEqual(exp_dynamic_waters_of_obstacle,
    #                      scenario_tmp.obstacle_by_id(2).prediction.center_waters_assignment)
    #     self.assertEqual(exp_dynamic_waters_of_obstacle,
    #                      scenario_tmp.obstacle_by_id(2).prediction.shape_waters_assignment)
    #     # assign one time step
    #     scenario_tmp: Scenario = deepcopy(sc)
    #     scenario_tmp.assign_obstacles_to_waters(time_steps=[1])
    #     exp_dynamic_obstacles_on_waters_zero = {1: {2}}
    #     exp_dynamic_waters_of_obstacle = {1: {1}}
    #     self.assertEqual(exp_dynamic_obstacles_on_waters_zero,
    #                      scenario_tmp.waters_network.find_waters_by_id(1).dynamic_obstacles_on_waters)
    #     self.assertEqual(None, scenario_tmp.obstacle_by_id(2).initial_center_waters_ids)
    #     self.assertEqual(exp_dynamic_waters_of_obstacle,
    #                      scenario_tmp.obstacle_by_id(2).prediction.center_waters_assignment)
    #     self.assertEqual(exp_dynamic_waters_of_obstacle,
    #                      scenario_tmp.obstacle_by_id(2).prediction.shape_waters_assignment)
    #     # assign center only
    #     scenario_tmp: Scenario = deepcopy(sc)
    #     scenario_tmp.assign_obstacles_to_waters(time_steps=None, use_center_only=True)
    #     exp_dynamic_obstacles_on_waters_zero = {0: {2}, 1: {2}}
    #     exp_dynamic_waters_of_obstacle = {0: {1}, 1: {1}}
    #     self.assertEqual(exp_dynamic_obstacles_on_waters_zero,
    #                      scenario_tmp.waters_network.find_waters_by_id(1).dynamic_obstacles_on_waters)
    ...
    #     self.assertEqual(exp_dynamic_waters_of_obstacle,
    #                      scenario_tmp.obstacle_by_id(2).prediction.center_waters_assignment)
    #     self.assertEqual(None, scenario_tmp.obstacle_by_id(2).prediction.shape_waters_assignment)
    ...
    #     scenario_tmp.assign_obstacles_to_waters(time_steps=None, obstacle_ids={3})
    #     exp_dynamic_obstacles_on_waters_zero = {0: {3}, 1: {3}}
    #     exp_dynamic_waters_of_obstacle = {0: {1}, 1: {1}}
    #     self.assertEqual(exp_dynamic_obstacles_on_waters_zero,
    #                      scenario_tmp.waters_network.find_waters_by_id(1).dynamic_obstacles_on_waters)
    #     self.assertEqual(None, scenario_tmp.obstacle_by_id(2).initial_center_waters_ids)
    #     self.assertEqual(None, scenario_tmp.obstacle_by_id(2).prediction.center_waters_assignment)
    #     self.assertEqual(None, scenario_tmp.obstacle_by_id(2).prediction.shape_waters_assignment)
    #     self.assertEqual(exp_dynamic_waters_of_obstacle[0], scenario_tmp.obstacle_by_id(3).initial_center_waters_ids)
    #     self.assertEqual(exp_dynamic_waters_of_obstacle,
    #                      scenario_tmp.obstacle_by_id(3).prediction.center_waters_assignment)
    #     self.assertEqual(exp_dynamic_waters_of_obstacle,
    #                      scenario_tmp.obstacle_by_id(3).prediction.shape_waters_assignment)
...
class TestScenarioID(unittest.TestCase):
    def test_from_benchmark_id(self):
        ...
        self.assertEqual(s_id.map_id, 33)
        self.assertEqual(s_id.configuration_id, 2)
        self.assertEqual(s_id.prediction_type, 'I')
...
class TestLocation(unittest.TestCase):
    def test_equality(self):
        ...
        environment = Environment(Time(2022, 1, 1, 8, 30), TimeOfDay.DAY, Weather.SUNNY, SeaState.ROUGH)
        ...
        geo_transformation = GeoTransformation('1234', 1.1, 1.2, 1.3, 1.4)
        environment = Environment(Time(2022, 1, 1, 8, 30), TimeOfDay.DAY, Weather.SUNNY, SeaState.UNKNOWN)
        location_2 = Location(123, 456, 789, geo_transformation, environment)
        self.assertFalse(location_1 == location_2)
...
class TestTime(unittest.TestCase):
    def test_equality(self):
        time_1 = Time(2022, 1, 1, 8, 30)
        time_2 = Time(2022, 1, 1, 8, 30)
        self.assertTrue(time_1 == time_2)

        time_2 = Time(2022, 1, 1, 9, 30)
        self.assertFalse(time_1 == time_2)

        time_2 = Time(2022, 1, 1, 8, 59)
        self.assertFalse(time_1 == time_2)
    ...
    def test_hash(self):
        ...
        time_2 = Time(2022, 1, 1, 8, 31)
        self.assertNotEqual(hash(time_1), hash(time_2))


class TestEnvironment(unittest.TestCase):
    def test_equality(self):
        environment_1 = Environment(Time(2022, 1, 1, 8, 30), TimeOfDay.DAY, Weather.SUNNY, SeaState.ROUGH)
        environment_2 = Environment(Time(2022, 1, 1, 8, 30), TimeOfDay.DAY, Weather.SUNNY, SeaState.ROUGH)
        self.assertTrue(environment_1 == environment_2)

        environment_2 = Environment(Time(2022, 1, 1, 9, 30), TimeOfDay.DAY, Weather.SUNNY, SeaState.ROUGH)
        self.assertFalse(environment_1 == environment_2)

        environment_2 = Environment(Time(2022, 1, 1, 8, 30), TimeOfDay.UNKNOWN, Weather.SUNNY, SeaState.ROUGH)
        self.assertFalse(environment_1 == environment_2)

        environment_2 = Environment(Time(2022, 1, 1, 8, 30), TimeOfDay.DAY, Weather.SNOW, SeaState.ROUGH)
        self.assertFalse(environment_1 == environment_2)

        environment_2 = Environment(Time(2022, 1, 1, 8, 30), TimeOfDay.DAY, Weather.SUNNY, SeaState.CALM)
        self.assertFalse(environment_1 == environment_2)
    ...
    def test_hash(self):
        ...
        environment_2 = Environment(Time(2022, 1, 1, 8, 30), TimeOfDay.NIGHT, Weather.SUNNY, SeaState.ROUGH)
