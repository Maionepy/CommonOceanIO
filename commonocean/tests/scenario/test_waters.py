...
from commonocean.scenario.waters import Waters, LineMarking, WatersNetwork #StopLine
from commonocean.scenario.obstacle import State, DynamicObstacle, ObstacleType
from commonocean.scenario.traffic_sign import TrafficSignElement, TrafficSign
...
class TestWaters(unittest.TestCase):
    def test_initialize_waters(self):
        ...
        waters = Waters(left_vertices, center_vertices, right_vertices, waters_id, predecessor, successor, None, None, None) # , {traffic_sign.traffic_sign_id})
        ...
        # for i, (min_dist, dist) in enumerate(zip(waters.inner_distance, waters.distance)):
        #     self.assertAlmostEqual(dist, desired_dist[i])
        #     self.assertLessEqual(min_dist, dist)
        #     print(min_dist, dist)

        self.assertEqual(waters.waters_id, waters_id)
        np.testing.assert_array_almost_equal(waters.right_vertices, right_vertices)
        np.testing.assert_array_almost_equal(waters.left_vertices, left_vertices)
        np.testing.assert_array_almost_equal(waters.center_vertices, center_vertices)
        np.testing.assert_array_almost_equal(waters.predecessor, predecessor)
        np.testing.assert_array_almost_equal(waters.successor, successor)
        # self.assertEqual(waters.adj_left, adjacent_left)
        # self.assertEqual(waters.adj_right, adjacent_right)
        # self.assertEqual(waters.adj_left_same_direction, adjacent_left_same_dir)
        # self.assertEqual(waters.adj_right_same_direction, adjacent_right_same_dir)
        # self.assertEqual(waters.line_marking_left_vertices, line_marking_left)
        # self.assertEqual(waters.line_marking_right_vertices, line_marking_right)
        # self.assertSetEqual(waters.traffic_signs, {traffic_sign.traffic_sign_id})
        # self.assertEqual(waters.stop_line, stop_line)

    def test_translate_rotate(self):
        right_vertices = np.array([[0, 0], [1, 0], [2, 0], [3, .5], [4, 1], [5, 1], [6, 1], [7, 0], [8, 0]])
        left_vertices = np.array([[0, 1], [1, 1], [2, 1], [3, 1.5], [4, 2], [5, 2], [6, 2], [7, 1], [8, 1]])
        center_vertices = np.array([[0, .5], [1, .5], [2, .5], [3, 1], [4, 1.5], [5, 1.5], [6, 1.5], [7, .5], [8, .5]])
        # stop_line = StopLine(start=np.array([0, 0]), end=np.array([0, 1]), line_marking=LineMarking.LAND)

        waters = Waters(left_vertices, center_vertices, right_vertices, 1, )#stop_line=stop_line)

        waters.translate_rotate(np.array([2, -4]), np.pi / 2)

        desired_waters_center = np.array(
        ...
        np.testing.assert_array_almost_equal(waters.center_vertices, desired_waters_center)
        # np.testing.assert_array_almost_equal(waters.stop_line.start, desired_stop_line_start)
        # np.testing.assert_array_almost_equal(waters.stop_line.end, desired_stop_line_end)
    ...
    def test_translate_invalid(self):
        ...
        with self.assertRaises(AssertionError):
            waters.translate_rotate([2, -4], 320)
        with self.assertRaises(AssertionError):
            waters.translate_rotate([3, 5, -7], np.pi / 2)
        with self.assertRaises(AssertionError):
            waters.translate_rotate([3], np.pi / 2)
        with self.assertRaises(AssertionError):
            waters.translate_rotate(0.0, np.pi / 2)
    ...
    #     interpolated_position = waters.interpolate_position(5.736067977)
    ...
    #     with self.assertRaises(AssertionError):
    #         waters.interpolate_position(10.0)
    #     # negative values should not be accepted
    #     with self.assertRaises(AssertionError):
    #         waters.interpolate_position(-1.0)
    ...
    def test_get_obstacles(self):
        ...
        dynamic_obs = DynamicObstacle(obstacle_id=30, obstacle_type=ObstacleType.ANCHOREDVESSEL, prediction=prediction,
                                      initial_state=State(
                                              **{'position': np.array([0, 2]), 'orientation': 0, 'time_step': 0}),
                                      obstacle_shape=rect)

        self.assertTrue(waters.get_obstacles([dynamic_obs]))
        self.assertTrue(waters.get_obstacles([dynamic_obs], time_step=2))
        self.assertFalse(waters.get_obstacles([dynamic_obs], time_step=3))
        self.assertTrue(waters.get_obstacles([dynamic_obs], time_step=4))
    ...
    def test_convert_to_polygon(self):
        ...
        polygon = waters.convert_to_polygon()
    ...
    # CommonOcean does not have a merging function
    ...
    #     lanelet_network = WatersNetwork.create_from_lanelet_list([lanelet1, lanelet2, lanelet3])
    ...
    # CommonOcean does not have a find waters by successors at the moment
    ...
    #     waters1 = Waters(v_left1, v_center1, v_right1, lanelet_id=1, successor=[2], predecessor=[3])
    #     waters2 = Waters(v_left2, v_center2, v_right2, lanelet_id=2, successor=[3], predecessor=[1])
    #     waters3 = Waters(v_left3, v_center3, v_right3, lanelet_id=3, successor=[1, 4], predecessor=[2])
    #     waters4 = Waters(v_left4, v_center4, v_right4, lanelet_id=4, successor=[], predecessor=[3])
    #     ln = WatersNetwork.create_from_lanelet_list([waters1, waters2, waters3, waters4])

    #     paths = waters1.find_lanelet_successors_in_range(ln)
    #     self.assertIn([2, 3], paths)
    #     self.assertIn([2, 3, 4], paths)
    #     self.assertTrue(len(paths) == 2)

    #     paths2 = waters1.find_lanelet_successors_in_range(ln, max_length=1.0)
    ...
    def test_equality(self):
        ...
        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id)
        self.assertTrue(waters1 == waters2)

        waters2 = Waters(np.array([[0, 1.00000000001], [1, 1], [8, 1]]), center_vertices, right_vertices, waters_id)
        self.assertTrue(waters1 == waters2)

        waters2 = Waters(np.array([[0, 1.0001], [1, 1], [8, 1]]), center_vertices, right_vertices, waters_id)
        self.assertFalse(waters1 == waters2)

        waters2 = Waters(left_vertices, np.array([[0, .5], [1, .49999999999], [8, .5]]), right_vertices, waters_id)
        self.assertTrue(waters1 == waters2)

        waters2 = Waters(left_vertices, np.array([[0, .5], [1, .6], [8, .5]]), right_vertices, waters_id)
        self.assertFalse(waters1 == waters2)

        waters2 = Waters(left_vertices, center_vertices, np.array([[0, 0], [1, 0], [7.999999999997, 0]]), waters_id)
        self.assertTrue(waters1 == waters2)

        waters2 = Waters(left_vertices, center_vertices, np.array([[0, 0], [1, 0], [7.7, 0]]), waters_id)
        self.assertFalse(waters1 == waters2)

        waters2 = Waters(left_vertices, center_vertices, right_vertices, 8)
        self.assertFalse(waters1 == waters2)
        ...
        waters1 = Waters(left_vertices, center_vertices, right_vertices, waters_id, predecessor, successor) #,
        #                     adjacent_left, adjacent_left_same_dir, adjacent_right, adjacent_right_same_dir)
        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id, predecessor, successor)#,
        #                    adjacent_left, adjacent_left_same_dir, adjacent_right, adjacent_right_same_dir)
        self.assertTrue(waters1 == waters2)

        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id, [3], successor)#, adjacent_left,
        #                    adjacent_left_same_dir, adjacent_right, adjacent_right_same_dir)
        self.assertFalse(waters1 == waters2)

        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id, [4, 3], successor)#,
        #                    adjacent_left, adjacent_left_same_dir, adjacent_right, adjacent_right_same_dir)
        self.assertTrue(waters1 == waters2)

        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id, predecessor, [2])#, adjacent_left,
        #                    adjacent_left_same_dir, adjacent_right, adjacent_right_same_dir)
        self.assertFalse(waters1 == waters2)

        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id, predecessor, [7, 8, 6])#,
        #                    adjacent_left, adjacent_left_same_dir, adjacent_right, adjacent_right_same_dir)
        self.assertTrue(waters1 == waters2)

        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id, predecessor, successor)#, 1,
        #                    adjacent_left_same_dir, adjacent_right, adjacent_right_same_dir)
        self.assertFalse(waters1 == waters2)
    ...
    def test_hash(self):
        ...
        waters1 = Waters(left_vertices, center_vertices, right_vertices, waters_id)
        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id)
        self.assertEqual(hash(waters1), hash(waters2))

        right_vertices = np.array([[0, 0], [1, 0.0000000001], [8, 0]])
        waters2 = Waters(left_vertices, center_vertices, right_vertices, waters_id)
        self.assertNotEqual(hash(waters1), hash(waters2))

# CommonOcean does not have a stopline function at the moment
