...
from commonocean.scenario.obstacle import DynamicObstacle, \
    ObstacleType, \
    StaticObstacle
from commonocean.visualization.util import approximate_bounding_box_dyn_obstacles
...
class TestUncertainStates(unittest.TestCase):
    ...
    # Here we need to have a file that can be used as an example

    # def test_draw(self):
    #     full_path = os.path.dirname(os.path.abspath(__file__))
    #     scenario_path = full_path + "/../test_scenarios/DEU_A9-3_1_T-1.xml"
    #     scenario, _ = CommonOceanFileReader(scenario_path).open()
    ...
    def _test_dynamic_obstacle(self, shape):
        ...
        prediction = TrajectoryPrediction(Trajectory(1, uncertain_states[1:]),
                                          shape)
        dyn_obs = DynamicObstacle(0, ObstacleType.MOTORVESSEL, shape,
    ...
    def _test_static_obstacle(self, shape):
        ...
        stat_obs = StaticObstacle(0, ObstacleType.MOTORVESSEL, shape, state)
