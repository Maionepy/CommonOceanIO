...
# from commonocean.scenario.traffic_sign import TrafficSignElementID, \
#     TrafficSign, TrafficSignElement

## For some reason, it is not importing draw_traffic_light_signs from commonocean
# But it does not seem it is necessary
# from commonocean.visualization.traffic_sign import text_prop_dict, draw_traffic_light_signs 
...
# In order to test this file properly, it is necessary to have the test_scenarios
...
class TestVisualization(unittest.TestCase):
    ...
    def test_scenario(self):
        ...
        scenario, planning_problem_set = CommonOceanFileReader(self.ngsim_scen_1).open()
    ...
    #     traffic_sign_types = [TrafficSignElementID]
    #     rnd = MPRenderer()
    ...
    def test_trajectory_unique_colors(self):
        scenario, planning_problem_set = CommonOceanFileReader(self.ngsim_scen_2).open()
    ...
    #         self.assertTrue(TrafficSignElementID[t_id].value in vis_dict,
    #                         f"Create plot settings for traffic sign {t_id} with ID {TrafficSignElementID[t_id].value}")
    ...
    def test_video(self):
        scenario, _ = CommonOceanFileReader(self.ngsim_scen_1).open()  #
        t0 = time.time()
    ...
    # Not applicable to commonocean at the moment
    ...
    #         print(full_path)
    #         scenario, planning_problem_set = CommonOceanFileReader(full_path).open()
    #         scenario.draw(rnd)
