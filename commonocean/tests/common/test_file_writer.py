...
from commonocean.common.file_writer import CommonOceanFileWriter, OverwriteExistingFile, float_to_str, Point, \
...
from commonocean.scenario.waters import Waters, WatersNetwork, LineMarking, WatersType
from commonocean.scenario.obstacle import *
from commonocean.scenario.scenario import Scenario, Tag, Location, ScenarioID

# from commonroad.common.file_writer import precision is not beeing imported (probably version of commonroad inside the env?)
...
class TestFileWriter(unittest.TestCase):
    ...
    # Not applicable as test scenarios have not been defined

    # def test_read_write_file(self):
    #     scenario_1, planning_problem_set_1 = CommonOceanFileReader(self.filename_read_1).open()
    #     filename = self.out_path + '/test_reading_intersection_traffic_sign.xml'
    #     CommonOceanFileWriter(scenario_1, planning_problem_set_1, scenario_1.author, scenario_1.affiliation, 'test',
    ...
    #     scenario_2, planning_problem_set_2 = CommonOceanFileReader(self.filename_read_2).open()
    #     filename = self.out_path + '/test_reading_all.xml'
    #     CommonOceanFileWriter(scenario_2, planning_problem_set_2, scenario_2.author, scenario_2.affiliation, 'test',
    ...
    #     scenario, planning_problem_set = CommonOceanFileReader(self.filename_2018b).open()
    #     filename = self.out_path + "/USA_Lanker-1_1_T-1.xml"
    #     CommonOceanFileWriter(scenario, planning_problem_set, scenario.author, scenario.affiliation,
    ...
    def test_writing_shapes(self):
        ...
        waters1 = Waters(np.array([[12345.12, 0.0], [1.0, 0.0], [2, 0]]), np.array([[0.0, 1], [1.0, 1], [2, 1]]),
                           np.array([[0.0, 2], [1.0, 2], [2, 2]]), 100, [101], [101], waters_type={WatersType.OPENSEA})
        waters2 = Waters(np.array([[0.0, 0.0], [1.0, 0.0], [2, 0]]), np.array([[0.0, 1], [1.0, 1], [2, 1]]),
                           np.array([[0.0, 2], [1.0, 2], [2, 2]]), 101, [100], [100], waters_type={WatersType.OPENSEA})

        waters_network = WatersNetwork().create_from_waters_list(list([waters1, waters2]))
        scenario = Scenario(0.1, ScenarioID.from_benchmark_id('ZAM_test_0-1', scenario_version=SCENARIO_VERSION))
        scenario.add_objects([static_obs, waters_network])
        ...
        CommonOceanFileWriter(scenario, planning_problem_set, 'PrinceOfZAM', 'TU Munich', 'test', {Tag.OPENSEA},
        ...
        CommonOceanFileWriter(scenario, planning_problem_set, 'PrinceOfZAM', 'TU Munich', 'test', {Tag.OPENSEA},
                             location).write_to_file(filename=filename,
                                                     overwrite_existing_file=OverwriteExistingFile.ALWAYS)
        CommonOceanFileWriter(scenario, planning_problem_set, 'PrinceOfZAM_no_problem', 'TU Munich', 'test', {Tag.OPENSEA},
    ...
    def test_float_to_str(self):
        ...
        cw = CommonOceanFileWriter(decimal_precision=3,
        ...
        f4 = 1e-23
        f5 = 0
        cw = CommonOceanFileWriter(decimal_precision=5,
                                  scenario=Scenario(dt=0.1, tags=set(), author="sdf", affiliation="",
                                                    source=""),
                                  planning_problem_set=PlanningProblemSet())
        str1 = float_to_str(f)
    ...
    # Future application, not developed for commonocean nor commonroad
    ...
    #         print(full_path)
    #         scenario_tmp, planning_problem_tmp = CommonOceanFileReader(full_path).open()
    #         CommonOceanFileWriter(scenario_tmp, planning_problem_tmp, 'PrinceOfZAM', 'TU Munich', 'unittest'). \
