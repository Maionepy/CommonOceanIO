...
from commonocean.scenario.scenario import Scenario, Tag, Location, GeoTransformation, SeaState, Weather, TimeOfDay, \
    ScenarioID
...
# Not possible to create yet, as test scenarios have not been defined
...
#         lanelets = CommonOceanFileReader(self.filename_lanelets).open()
...
#         obstacles = CommonOceanFileReader(self.filename_obstacle).open()
...
#         planning_problem = CommonOceanFileReader(self.filename_planning_problem).open()
...
#         exp_location_env_seastate = SeaState.CALM
...
#         xml_file = CommonOceanFileReader(self.filename_all).open(lanelet_assignment=True)
...
#         self.assertEqual(exp_location_env_seastate, xml_file[0].location.environment.seastate)
...
#         xml_file = CommonOceanFileReader(self.filename_urban).open()
...
#         xml_file = CommonOceanFileReader(self.filename_all).open(lanelet_assignment=False)
...
#     #         CommonOceanFileReader(full_path).open(lanelet_assignment=True)
#     #
