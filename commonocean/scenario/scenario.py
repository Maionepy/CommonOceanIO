...
from commonocean.scenario.obstacle import StaticObstacle, DynamicObstacle, State, ObstacleRole, ObstacleType
...
class Scenario:
    ...
    def waters_network(self) -> WatersNetwork:
        """ Road network composed of waters."""
        return self._waters_network

    @waters_network.setter
    def waters_network(self, waters_network: WatersNetwork):
        assert isinstance(waters_network, WatersNetwork), '<Scenario/waters_network> argument "waters_network"' \
                                                            ' of wrong type. Expected type: %s. Got type: %s.' \
                                                          % (WatersNetwork, type(waters_network))
        self._waters_network = waters_network
    ...
    def add_objects(self, scenario_object: Union[List[Union[Obstacle, Waters, WatersNetwork, TrafficSign,
                                                            TrafficLight, Draft]], Obstacle, Waters, WatersNetwork,
                                                 TrafficSign, TrafficLight, Draft], waters_ids: Set[int] = None):
        """ Function to add objects, e.g., waters, dynamic and static obstacles, to the scenario.
            ...
            :param waters_ids: water IDs a traffic sign, traffic light should be referenced from
        ...
        if isinstance(scenario_object, list):
            ...
            #                                      scenario_object.initial_shape_waters_ids)
            ...
            self.waters_network: WatersNetwork = scenario_object
            ...
            self._waters_network.add_traffic_light(scenario_object, waters_ids)
        elif isinstance(scenario_object, Draft):
            self._drafts.append(scenario_object)
        else:
    ...
    def remove_obstacle(self, obstacle: Union[Obstacle, List[Obstacle]]):
        ...
        if obstacle.obstacle_id in self._static_obstacles:
            # self._remove_static_obstacle_from_lanelets(obstacle.obstacle_id, obstacle.initial_shape_waters_ids)
    ...
    def drafts(self) -> List[Draft]:
        """ Returns a list of all drafts in the scenario."""
        return self._drafts
    ...
    def assign_obstacles_to_lanelets(self, time_steps: Union[List[int], None] = None,
        ...
        """
        Assigns center points and shapes of obstacles to waters by setting the attributes
        Obstacle.prediction.initial_shape_waters_ids, .shape_lanelet_assignment, .initial_center_waters_ids,
        & .center_lanelet_assignment, and Lanelet.dynamic_obstacles_on_water & .static_obstacles_on_water.
        ...
        :param use_center_only: if False, the shape is used to find occupied waters.
        ...
        def assign_dynamic_obstacle_shape_at_time(obstacle: DynamicObstacle, time_step):
            ...
            lanelet_ids_center = set(self.waters_network.find_lanelet_by_position([position])[0])
            ...
            if use_center_only:
                ...
                lanelet_ids = set(self.waters_network.find_water_by_shape(shape))
                obstacle.prediction.shape_lanelet_assignment[time_step] = lanelet_ids

            if time_step == obstacle.initial_state.time_step:
                if not use_center_only:
                    obstacle.initial_shape_waters_ids = lanelet_ids
                obstacle.initial_center_waters_ids = lanelet_ids_center
            for l_id in lanelet_ids:
                self.waters_network.find_lanelet_by_id(l_id) \
        ...
        def assign_static_obstacle(obstacle: StaticObstacle):
            ...
            if not use_center_only:
                lanelet_ids = set(self.waters_network.find_water_by_shape(shape))
                obstacle.initial_shape_waters_ids = lanelet_ids
            lanelet_ids = set(self.waters_network.find_lanelet_by_position([obstacle.initial_state.position])[0])
            obstacle.initial_center_waters_ids = lanelet_ids
    ...
    def __str__(self):
        ...
        traffic_str += "- Number of Drafts: {}\n".format(len(self.drafts))
        traffic_str += "- Waters:\n"
